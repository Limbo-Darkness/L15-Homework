...
##fetch_title function with similar structure to fetch_url
#
def fetch_title():
    url = _url.get()
    try:
        ...
        soup = BeautifulSoup(page.content, 'html.parser')
        webtitle = soup.find('title') #sorts through
        stringtitle = str(webtitle) #converts the found webtitle into a string for use with string functions
        webtitlelength = len(stringtitle)
        correctedwebtitle = stringtitle[7:webtitlelength-8] #because html format requires <title>&</title>,
        #this is used to extract the content
        if webtitle:
            #appends together the formatting and website title
            titlefound = 'Title Found: [\''
            titlefound+=correctedwebtitle
            titlefound+='\']'
            #updates status bar
            sb(titlefound)
        else:
...
##fetch_link function with similar structure to fetch_url
#
def fetch_link():
    url = _url.get()
    config['urls'] = [] #initializes empty list
    _images.set(())  # initialised as an empty tuple
    try:
        page = requests.get(url)
    except requests.RequestException as err:
        ...
        soup = BeautifulSoup(page.content, 'html.parser')
        urls = fetch_urls(soup, url) #calls fetch_urls function similar to fetch_url process
        if urls:
            _images.set(tuple(urls)) #places external urls in the content field

##fetch_urls function used in fetch_link to find external urls and return a list of sorted urls
#
def fetch_urls(soup, base_url):
    urls = soup.findAll('a') #soup function to sort through the first part of html
    sortedurls = [] #initializes empty list
    for link in urls:
        href = link['href'] #takes the content out of the href attribute
        stringlink = str(href) #converts href to a string for use with further sorting
        if stringlink.startswith('http'): #ensures content is a link
            if stringlink.startswith(base_url): #continues if the url contains part of the base url
                continue
            sortedurls.append(stringlink) #adds to sorted list if all conditions are met
    return sortedurls
...
if __name__ == "__main__": # execute logic if run directly
    ...
    _root.title('COP2080 L15 Scrape App')
    ...
    ##Fetch title button
    #
    _fetchtitle_btn = ttk.Button(
        _url_frame, text='Fetch title', command=fetch_title)
    _fetchtitle_btn.grid(row=1, column=1, sticky=W, padx=5)

    ##Fetch link button
    #
    _fetchlink_btn = ttk.Button(
        _url_frame, text='Fetch link', command=fetch_link)
    _fetchlink_btn.grid(row=2, column=1, sticky=W, padx=5)
    ##
